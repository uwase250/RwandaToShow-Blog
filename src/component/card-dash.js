...
const Cardassh = ({ Data, id }) => {
  ...
  const deleteRecord = async (id) => {
    ...
    try {
      console.log("Cliked");
      const deleteRecords = await axios.delete(
        `https://blog-6hj4.onrender.com/api/post/delete/${id}`,
        {
          headers: {
  ...
  const updateRecord = async (_id) => {
    ...
    try {
      const updateResponse = await axios.put(
        `https://blog-6hj4.onrender.com/api/post/update/${id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
